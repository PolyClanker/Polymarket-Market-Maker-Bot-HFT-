...
from utils.position_redeemer import redeem_market_positions, merge_balanced_positions
from config import (
    MAX_TRADES,
...
def main():
    ...
    logger.info(f"Using Private Key: {os.getenv('PRIVATE_KEY')} and Proxy Address: {os.getenv('POLYMARKET_PROXY_ADDRESS')}")
    set_cpu_affinity()
    ...
    while True:
        if not is_in_trading_window():
            ...
            # Step 1: Merge balanced positions immediately (doesn't require resolution)
            # This recovers the "neutral" portion of the book instantly
            logger.info("💰 Merging balanced positions for immediate capital recovery...")
            merge_balanced_positions()
            
            # Step 2: Try to redeem any remaining positions (requires resolution)
            # Unbalanced positions (the "gambling" part) will wait for resolution
            logger.info("Checking for redeemable positions...")
            redeem_market_positions(market_slug)
        ...
        down_ask_price = 1 - up_bid_price
        down_bid_price = 1 - up_ask_price
        micro_bps = market_data["micro_vs_mid_bps"]

        if not ((0.2 < up_ask_price < 0.35) or (0.65 < up_bid_price < 0.8)) or (
            abs(micro_bps) > MAX_TRADING_BPS_THRESHOLD
        ):
            continue
